...
export default function Features() {
  ...
  return (
    <section className="py-20 px-4 bg-white section-soft">
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        {features.map((f, idx) => (
          <div
            key={f.title}
            className={`bg-orange-50 border border-orange-100 rounded-xl p-8 text-center hover:border-orange-300 lift-hover transition-all duration-500 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}
